...
class IntensityODEFunc(nn.Module):
    ...
    def forward(self, t, state):
        tpp_state = state
        return self.dstate_fn(t, tpp_state)
...
class NeuralODE_RNN(TemporalPointProcess):
    ...
    def logprob(self, event_times, spatial_locations, input_mask, t0, t1):
        final_state = self.integrate_lambda(event_times, spatial_locations, input_mask, t0, t1)
    ...
    def integrate_lambda(self, event_times, spatial_location, input_mask, t0, t1, nlinspace=1):
        ...
        for i in range(T):
            ...
            hiddens = state_traj  # (1 + nlinspace, N, D)
            if i > 0:
                hiddens = hiddens[1:]
...
class TimeVariableODE(nn.Module):
    ...
    def forward(self, s, state):
        ...
        return tuple([torch.zeros_like(t0), torch.zeros_like(t1), dx])
